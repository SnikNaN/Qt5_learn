...
{
    QUrl url(m_ptxt->text());
    if (url.scheme().isEmpty())
    {
        url.setScheme("http");
        url.setUrl("http://" + url.host() + url.path());
    }
    m_pdl->download(url);
    QHostInfo::lookupHost(url.authority(), this, SLOT(slotHost(QHostInfo)));
}
...
void DownloaderGui::slotHost(QHostInfo info)
{
