...
SoundPlayer::SoundPlayer(QWidget* pwgt/*=0*/) : QWidget(pwgt)
{
    ...
    DialSpeed*   pdiaSpeed  = new DialSpeed;
    ...
    pdiaVolume->setProperty("toolTip", "Volume");
    pdiaSpeed->setProperty("toolTip", "Speed");

    pdiaVolume->setRange(0, 100);
    pdiaSpeed->setRange(3, 30);
    int nDefaultVolume = 50;
    int nDefaultSpeed = 10;
    m_pmp->setVolume(nDefaultVolume);
    pdiaVolume->setValue(nDefaultVolume);
    pdiaSpeed->setValue(nDefaultSpeed);

    connect(pdiaSpeed, SIGNAL(value_change(qreal)), m_pmp, SLOT(setPlaybackRate(qreal)));

    connect(pdiaSpeed, SIGNAL(valueChanged(int)), pdiaSpeed, SLOT(changed(int)));
    ...
    phbxPlayControls->addWidget(pdiaSpeed);
